...
try {
    ...
    foreach ($grades_data['grades'] as $index => $grade_item) {
        if (!isset($grade_item['homework_type_id']) || !isset($grade_item['homework_grades_id'])) {
            echo json_encode([
                'success' => false,
                'message' => "Missing homework_type_id or homework_grades_id in grade item at index $index"
            ]);
            exit();
        }
        ...
        $grades_data['grades'][$index]['homework_grades_id'] = (int)$grade_item['homework_grades_id'];
    ...
    $result = $homework->addGradeHomework($grades_data);
    ...
    error_log("Add grade homework API error: " . $e->getMessage());
    echo json_encode([
        'success' => false,
